...
class HistoriqueSociete(models.Model):
    ...
    def import_file(type_file, file):
        ...
        source = '/upload/societe/'
        if type_file == '1':
            nom = 'logo'
            new_historique.logo = file
            content = "<img src='" + source + str(new_historique.logo) + "' width=150 alt=' Fichier téléchargeable' />"
            content += "<br><a class='btn btn-success' href='" + source + str(new_historique.logo) + \
                       "'>Télécharger le fichier</a>"
        elif type_file == '2':
            nom = 'scan nif'
            new_historique.scan_nif = file
            content = "<img src='" + source + str(new_historique.scan_nif) + "' width=150 alt=' Fichier " \
                                                                             "téléchargeable' />"
            content += "<br><a class='btn btn-success' href='" + source + str(new_historique.scan_nif) + \
                       "'>Télécharger le fichier</a>"
        elif type_file == '3':
            nom = 'scan stat'
            new_historique.scan_stat = file
            content = "<img src='" + source + str(new_historique.scan_stat) + "' width=150 alt=' Fichier " \
                                                                              "téléchargeable' />"
            content += "<br><a class='btn btn-success' href='" + source + str(new_historique.scan_stat) + \
                       "'>Télécharger le fichier</a>"
        elif type_file == '4':
            nom = 'scan registre de commerce'
            new_historique.scan_reg_commerce = file
            content = "<img src='" + source + str(new_historique.scan_reg_commerce) + "' width=150 alt=' Fichier " \
                                                                                      "téléchargeable' />"
            content += "<br><a class='btn btn-success' href='" + source + str(new_historique.scan_reg_commerce) + \
                       "'>Télécharger le fichier</a>"
        else:
